...
QVector<EGLint> EglUtils::createConfigAttributesFromFormat(const QSurfaceFormat &format)
{
...
bool EglUtils::reduceConfigAttributes(QVector<EGLint> *configAttributes)
{
    int i = -1;
...
EGLConfig EglUtils::configFromGLFormat(EGLDisplay display, const QSurfaceFormat &format, bool highestPixelFormat, int surfaceType)
{
    EglConfigChooser chooser(display);
...
QSurfaceFormat EglUtils::glFormatFromConfig(EGLDisplay display, const EGLConfig config, const QSurfaceFormat &referenceFormat)
{
...
bool EglUtils::hasEglExtension(EGLDisplay display, const char* extensionName)
{
...
void EglUtils::printEglConfig(EGLDisplay display, EGLConfig config)
{
    EGLint index;
...
QSize EglUtils::screenSizeFromFb(int framebufferDevice)
{
#ifndef Q_OS_LINUX
    Q_UNUSED(framebufferDevice)
#endif
    ...
    if (size.isEmpty()) {
        int width = qEnvironmentVariableIntValue("GREENISLAND_QPA_WIDTH");
        int height = qEnvironmentVariableIntValue("GREENISLAND_QPA_HEIGHT");
        ...
        if (framebufferDevice != -1) {
            if (ioctl(framebufferDevice, FBIOGET_VSCREENINFO, &vinfo) == -1) {
                qCWarning(lcEglConvenience, "Could not read screen info");
            } else {
                xres = vinfo.xres;
                yres = vinfo.yres;
            }
...
int EglUtils::screenDepthFromFb(int framebufferDevice)
{
#ifndef Q_OS_LINUX
    Q_UNUSED(framebufferDevice)
#endif
    const int defaultDepth = 32;
    static int depth = qEnvironmentVariableIntValue("GREENISLAND_QPA_DEPTH");
...
qreal EglUtils::refreshRateFromFb(int framebufferDevice)
{
    static qreal rate = 0;
...
EglConfigChooser::EglConfigChooser(EGLDisplay display)
    : m_display(display)
...
EGLConfig EglConfigChooser::chooseConfig()
{
    QVector<EGLint> configureAttributes = EglUtils::createConfigAttributesFromFormat(m_format);
    ...
    if (needsES2Plus) {
        if (m_format.majorVersion() >= 3 && EglUtils::hasEglExtension(display(), "EGL_KHR_create_context"))
    ...
    } while (EglUtils::reduceConfigAttributes(&configureAttributes));
...
bool EglConfigChooser::filterConfig(EGLConfig config) const
{
