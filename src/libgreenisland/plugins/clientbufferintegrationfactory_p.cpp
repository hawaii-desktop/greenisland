...
ClientBufferIntegrationInterface *ClientBufferIntegrationFactory::loadPlugin()
{
    // Detect the plugin from the environment, or fallback to the default
    QByteArray pluginToLoad = qgetenv("QT_WAYLAND_HARDWARE_INTEGRATION");
    if (pluginToLoad.isEmpty())
        pluginToLoad = qgetenv("QT_WAYLAND_CLIENT_BUFFER_INTEGRATION");
    if (pluginToLoad.isEmpty())
        pluginToLoad = QByteArrayLiteral("wayland-egl");

    const QStringList paths = QCoreApplication::libraryPaths();
    qCDebug(GREENISLAND_COMPOSITOR) << "Client buffer integration plugin lookup paths:" << qPrintable(paths.join(' '));

    Q_FOREACH (const QString &path, paths) {
        const QDir dir(path + QStringLiteral("/greenisland/hardware-integration/client/"),
                       QStringLiteral("*.so"),
        ...
        Q_FOREACH (const QFileInfo &info, infoList) {
            ...
            ClientBufferIntegrationInterface *plugin = qobject_cast<ClientBufferIntegrationInterface *>(instance);
            if (plugin) {
