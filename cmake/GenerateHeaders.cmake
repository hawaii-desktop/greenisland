...
function(GENERATE_HEADERS camelcase_forwarding_headers_var)
    set(options)
    set(oneValueArgs ORIGINAL OUTPUT_DIR PREFIX REQUIRED_HEADERS COMMON_HEADER RELATIVE)
    set(multiValueArgs HEADER_NAMES PRIVATE_HEADERS)
    cmake_parse_arguments(EGH "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

    if(EGH_UNPARSED_ARGUMENTS)
        message(FATAL_ERROR "Unexpected arguments to GENERATE_HEADERS: ${EGH_UNPARSED_ARGUMENTS}")
    endif()

    if(NOT EGH_HEADER_NAMES)
       message(FATAL_ERROR "Missing header_names argument to GENERATE_HEADERS")
    endif()
    ...
    if(NOT EGH_ORIGINAL STREQUAL "LOWERCASE" AND NOT EGH_ORIGINAL STREQUAL "CAMELCASE")
        message(FATAL_ERROR "Unexpected value for original argument to GENERATE_HEADERS: ${EGH_ORIGINAL}")
    endif()
    ...
    foreach(_privateheader ${EGH_PRIVATE_HEADERS})
        set(PRIVATE_HEADER_FILE "${EGH_OUTPUT_DIR}/${EGH_PREFIX}private/${_privateheader}")

        set(_actualheader "${CMAKE_CURRENT_SOURCE_DIR}/${EGH_RELATIVE}${_privateheader}")
        if(NOT EXISTS ${_actualheader})
            message(FATAL_ERROR "Could not find \"${_actualheader}\"")
        endif()

        if(NOT EXISTS ${PRIVATE_HEADER_FILE})
            file(WRITE ${PRIVATE_HEADER_FILE} "#include \"${_actualheader}\"\n")
        endif()
